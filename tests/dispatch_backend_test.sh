...
# exec with local backend args splices "-p local-headless" into the codex argv
( cd "$repo" && CODEX_DISPATCH_CODEX_BIN="$fake" FAKE_CODEX_ARGV_LOG="$log" \
    bash -c 'source "'"$PS_REPO_ROOT"'/lib/jsonutil.sh"; source "'"$PS_REPO_ROOT"'/lib/dispatch.sh";
             tmp="$(mktemp)"; d_codex_exec "backend-test-local" "'"$repo"'" "$tmp" "do it" -p local-headless >/dev/null; rm -f "$tmp"' )
assert_contains "$(cat "$log")" "-p local-headless" "exec splices backend args"
...
# --- retries keep the backend flag (resume must also carry -p local-headless) --------
: > "$log"
...
# the retry path calls `codex exec resume ... -p local-headless`; assert a resume line carries the flag
assert_contains "$(grep resume "$log" || true)" "-p local-headless" "retry resume carries backend flag"
